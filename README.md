...
│
├── ry/                                   ← 脚本A 产物 (不再纳入 Git，直接上传 R2)
├── tvbox/                                ← 脚本B 产物 (不再纳入 Git，直接上传 R2)
│   ├── *.json                            ← 各线路接口文件
│   ├── 海量直播线路.json                  ← 脚本C 聚合锚点
│   └── live/                             ← 脚本C 播放列表输出
│       ├── xxx.m3u
│       └── xxx.txt
├── livelist.txt                          ← 脚本C 索引清单 (不再纳入 Git，直接上传 R2)
├── list.txt                              ← 脚本B 接口清单 (不再纳入 Git，直接上传 R2)
├── SUMMARY.txt                           ← 脚本B 运行摘要 (不再纳入 Git，直接上传 R2)
│
...
三个工作流位于 .github/workflows/，推送到 main 分支后 GitHub 自动识别。三者串行执行，最后同步至 R2。
...
· 提交产物：调用 python/upload_r2.py，利用 S3 API 将数据直接推送到 Cloudflare R2，实现源码与数据解耦。
...
python3 python/upload_r2.py # 本地推送至 R2 测试
```
...
2. 创建项目，关联 GitHub 仓库 HPCstan/tvbox-api-backup。
3. 构建设置：
...
R2 上传失败 密钥未配置 检查 Repo Secrets 是否设置正确
混合内容破图 http 图片在 https 下拦截 引入 wsrv.nl 代理中转
...
改产物目录 脚本 OUTPUT_DIR + upload_r2.py ✅ push 后
...
项目：海量接口搬运备份站
仓库：https://github.com/HPCstan/tvbox-api-backup